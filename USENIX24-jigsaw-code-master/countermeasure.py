...
def padding_ddrsse(real_query_d, bkt=400):
    N_query, N_doc = real_query_d.shape
    real_query_d_new = np.zeros((N_query, N_doc*2))
    for query_idx in range(N_query):
        match_indices = sorted(np.where(real_query_d[query_idx] == 1))

        rand_b = random.randint(0,1)
        rand_r = random.randint(0, len(match_indices))
        ...
        if rand_b == 0:
            for idx in range(len(match_indices)):
                ...
                if idx < rand_r:
                    real_query_d_new[query_idx][match_idx] = 1
                else:
                    real_query_d_new[query_idx][N_doc + match_idx] = 1
