...
def get_all_for_attacks(
    ...
    ## Bucketization for DDR-SSE on the target database and auxiliary database
    kw_counts_real = {}
    kw_counts_sim = {}    
    if countermeasure_params["alg"] == "ddr-sse":
        ## Count keyword frequencies in the target database
        kw_counts_real = {}
        for kw in known_kws:
            kw_counts_real[kw] = 0
        for doc in real_doc:
        ...
        ## Sorting
        kw_real_sorted = sorted(kw_counts_real.keys(), key=lambda x:kw_counts_real[x], reverse=True)

        ## Padding to the documents directly
        bkt = countermeasure_params["bkt"]
        for ctr, kw in enumerate(kw_real_sorted):
            count_current = kw_counts_real[kw]
            target_idx    = ctr // bkt * bkt
            count_target  = kw_counts_real[kw_real_sorted[target_idx]]

            while count_current < count_target:
                doc_id = random.randint(0, len(real_doc)-1)
                if kw not in real_doc[doc_id]:
                    real_doc[doc_id] += [kw]
                    count_current += 1
        
        ## Count keyword frequencies in the auxiliary database
        kw_counts_sim = {}
        for kw in user_kws_universe:
            kw_counts_sim[kw] = 0
        for doc in sim_doc:
            for kw in doc:
                if kw in user_kws_universe:
                    kw_counts_sim[kw] += 1

        ## Sorting
        kw_sim_sorted = sorted(kw_counts_sim.keys(), key=lambda x:kw_counts_sim[x], reverse=True)

        ## Padding to the documents directly
        for ctr, kw in enumerate(kw_sim_sorted):
            count_current = kw_counts_sim[kw]
            target_idx    = ctr // bkt * bkt
            count_target  = kw_counts_sim[kw_sim_sorted[target_idx]]

            while count_current < count_target:
                doc_id = random.randint(0, len(sim_doc)-1)
                if kw not in sim_doc[doc_id]:
                    sim_doc[doc_id] += [kw]
                    count_current += 1
    ...
    if countermeasure_params["alg"] == "padding_linear":
        ...
        real_query_d = padding_ddrsse(real_query_d,countermeasure_params["bkt"])
