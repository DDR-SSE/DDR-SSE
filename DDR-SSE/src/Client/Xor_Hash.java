...
public class Xor_Hash {
    ...
    public void XorMM_setup(KV[] kv_list, int element_size, int level) throws Exception {
        int table_size = (int) Math.floor(((element_size*1.23)+beta)/3);
        EMM = new byte[table_size*3][];
        enc_list = new byte[kv_list.length][];
        ...
        for (int i = reverseOrderPos - 1; i >= 0; i--) {
            ...
            for (int hi = 0; hi < HASHES; hi++) {
                ...
                if (found == hi) {
                    ...
                    if(EMM[h]==null) {
                        EMM[h] = Hash.Get_SHA_256(longToBytes(random.nextInt(10000)));
                    }
