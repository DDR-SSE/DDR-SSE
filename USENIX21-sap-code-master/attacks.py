...
class Attack:
    ...
    def process_traces(self, traces):
        ...
        elif self.def_name in ('ppyy', 'sealvol', 'ddr-sse'):
...
class HungAttack(Attack):
    ...
    def _build_cost_vol(self, dumb_flag):
        keyword_counter_train = Counter([kw for document in self.training_dataset for kw in document])
        # padding if the DDR-SSE is used
        if self.def_name in ('ddr-sse',):
            sorted_keywords = sorted(keyword_counter_train.keys(), key=lambda x: keyword_counter_train[x], reverse=True)
            bucket_size = self.def_params[0]
            for ctr, kw in enumerate(keyword_counter_train):
                target_keyword = sorted_keywords[(ctr//bucket_size)*bucket_size]
                keyword_counter_train[kw] = keyword_counter_train[target_keyword]
        ...
        if self.def_name in ('ppyy', 'sealvol', 'ddr-sse'):
            kw_counts_test = [self.tag_info[tag] for tag in self.tag_info]
        else:
            kw_counts_test = [len(self.tag_info[tag]) for tag in self.tag_info]

        # Computing the cost matrix
        if dumb_flag or self.def_name in ('none', 'ddr-sse'):
    ...
    def _build_cost_freq(self, dumb_flag):
        if dumb_flag or self.def_name in ('none', 'osse', 'clrz', 'ppyy', 'debug', 'sealvol', 'ddr-sse'):
