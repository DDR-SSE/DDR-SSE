...
def generate_observations(full_data_client, def_params, real_queries):
    ...
    elif def_params['name'] == 'ddr-sse':
        # apply buketization
        bucket_size = def_params['bkt']
        sorted_kw_ids = sorted(inverted_index.keys(), key=lambda x: len(inverted_index[x]), reverse=True)

        for kw_id in inverted_index:
            inverted_index[kw_id] = set(inverted_index[kw_id])

        for ctr, kw_id in enumerate(sorted_kw_ids):
            target_kw_id = sorted_kw_ids[(ctr//bucket_size)*bucket_size]
            while len(inverted_index[kw_id]) < len(inverted_index[target_kw_id]):
                inverted_index[kw_id].add(random.randint(0, doc_id_max-1))

        for kw_id in inverted_index:
            inverted_index[kw_id] = sorted(inverted_index[kw_id])

        # apply duplication to the query responses
        query_responses = {}
        for kw_id in set(real_queries):
            side = random.randint(0,1)
            split_idx = random.randint(0, len(inverted_index[kw_id]))

            if side == 0:
                query_responses[kw_id] =  [x for x in inverted_index[kw_id][:split_idx]]
                query_responses[kw_id] += [x+doc_id_max for x in inverted_index[kw_id][split_idx:]]
            else:
                query_responses[kw_id] =  [x+doc_id_max for x in inverted_index[kw_id][:split_idx]]
                query_responses[kw_id] += [x for x in inverted_index[kw_id][split_idx:]]
            

        trace_type = 'ap_ddrsse'
        ndocs_retrieved = 0
        ndocs_real = 0
        token_ids = np.random.permutation(np.max(real_queries) + 1)
        for kw_id in real_queries:
            traces.append((token_ids[kw_id], query_responses[kw_id]))
            ndocs_retrieved += len(query_responses[kw_id])
