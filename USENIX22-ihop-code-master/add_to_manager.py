...
def add_to_manager5():
    """Modified from add_to_manager4(), targetting DDR-SSE"""

    manager_filename = 'manager_data5.pkl'
    if not os.path.exists(manager_filename):
        manager = Manager()
    else:
        with open(manager_filename, 'rb') as f:
    ...
    #dataset_and_pct_list = (('enron-full', (50, 25, 10)),
    #                        ('lucene', (50, 25, 10)))
    #def_list = [('none', ()), ('ddr-sse', 100), ('ddr-sse', 200), ('ddr-sse', 400)]
    def_list = [('none', ()), ('clrz', (0.999, 0.1)), ('ddr-sse', 400)]
    dataset_and_pct_list = (('enron-full', (50,)),)

    for def_params in def_list:
    #for def_name in ['ddr-sse']:

        if def_params[0] == 'none':
            exp_params.set_defense_params(def_params[0])
        if def_params[0] == 'clrz':
            exp_params.set_defense_params(def_params[0], tpr=def_params[1][0], fpr=def_params[1][1])
        if def_params[0] == 'ddr-sse':
            exp_params.set_defense_params(def_params[0], bkt=def_params[1])
