...
def get_candidate_assignments(leakage, aux_info):
    ...
    for idx_tar in range(len(leakage['keywords'])):
        ...
        for idx_aux in range(len(padded_counts_aux_range)):
    ...
    for idx_tar in range(len(leakage['keywords'])):
        if len(candidate_assignments[idx_tar]) == 0:
            candidate_assignments[idx_tar] = [ii for ii in range(len(padded_counts_aux_range))]
...
def initial_assignment(candidate_assignments, aux_info):
    free_pool = set([ii for ii in range(len(aux_info['comatrix']))])
...
def get_score_row(idx, assignments, leakage, aux_info):
    ...
    for idx2 in range(leakage['comatrix'].shape[0]):
        ...
        lambda_aux_min = int(lambda_aux - 3*np.sqrt(lambda_aux))
        lambda_aux_max = int(lambda_aux + 3*np.sqrt(lambda_aux))

        score_local = 0
        if count_obs <= lambda_aux_max:
            score_local += ((1/(lambda_aux_min+1)) + (1/(lambda_aux_max+1))) / 2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nkw',    dest='N_keywords',  action='store',     type=int, help="Specify the number of keywords, default is 1,000.")
    parser.add_argument('--nbkt',   dest='bucket_size', action='store',     type=int, help="Specify the bucket size, default is 200.")
    parser.add_argument('--nqry',   dest='N_queries',   action='store',     type=int, help="Specify the number of queries, default is 400.")
    parser.add_argument('--nitr',   dest='N_iters',     action='store',     type=int, help="Specify the number of iterations to run the attack, default is 200,000.")

        
    args = parser.parse_args(sys.argv[1:])

    N_docs          = args.N_docs if args.N_docs != None else 200000
    N_keywords      = args.N_keywords if args.N_keywords != None else 1000
    bucket_size     = args.bucket_size if args.bucket_size != None else 200
    N_queries       = args.N_queries if args.N_queries != None else 100
    N_iters         = args.N_iters if args.N_iters != None else 500000
    ...
    try:
        file_input = open(path_input + f'aux_info_{N_docs}_{N_keywords}.pkl', 'rb')
    ...
    try:
        leakage = process_leakage(path_input + f'leakage_{N_docs}_{N_keywords}_{bucket_size}_{N_queries}.txt')
    ...
    filename_output = path_output+f"result_{N_docs}_{N_keywords}_{bucket_size}_{N_queries}"
