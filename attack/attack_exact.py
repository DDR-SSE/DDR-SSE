...
def process_leakage(filename_input):
    ...
    for idx1 in range(len(responses)):
        for idx2 in range(idx1, len(responses)):
            comatrix[idx1, idx2] = len(responses[idx1].intersection(responses[idx2]))
            comatrix[idx2, idx1] = comatrix[idx1, idx2]
...
def get_candidate_assignments(leakage, aux_info):
    ...
    for idx in range(len(leakage['keywords'])):
        cocount_max_leakage += [max(max(leakage['comatrix'][idx, :idx]), max(leakage['comatrix'][idx, idx+1:]))]
        cocount_max_aux += [max(max(aux_info['comatrix'][idx, :idx]), max(aux_info['comatrix'][idx, idx+1:]))]
    ...
    for idx_tar in range(len(leakage['keywords'])):
        ...
        for idx_aux in range(len(padded_counts_aux_range)):
            if count_tar == padded_counts_aux[idx_aux] and cocount_max_leakage[idx_tar] <= cocount_max_aux[idx_aux]+20:
...
if __name__ == "__main__":
    ...
    filename_output = path_output+f"result_{N_docs}_{N_queries}_{bucket_size}_{percentile}.txt"
