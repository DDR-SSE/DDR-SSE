...
def build_inverted_index(path_input, include_keywords, N_files):
    ...
    target_indices = [ii for ii in range(N_files)]
    random.shuffle(target_indices)
    target_indices = set(target_indices[:N_files//2])
            ...
            for filename in os.listdir(os.path.join(path_input, folder, sub_folder)):
                ...
                if os.path.isfile(os.path.join(path_input, folder, sub_folder, filename)):
                    ...
                    if file_ctr in target_indices:
                        ...
                        for chunk_idx in range(dup_counter):
                            for keyword in keywords:
                                inverted_index_aux[keyword] += [file_ctr3]
                            file_ctr3 += 1
                    ...
                    if file_ctr >= N_files:
                        return inverted_index_target, inverted_index_aux, documents_target
